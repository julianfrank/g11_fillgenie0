...
const Step3_PVConfig = () => {
    ...
    return (
        <div className="glass-panel p-4 rounded-3 mb-4">
            ...
            <div className="mb-3">
                <label className="form-label text-light" title="The inverter is the device that changes the DC electricity (raw power from panels) into AC electricity (usable power for the grid). The 'topology' is the strategy we use to connect them. 'String' means we chain many panels together like old Christmas lights—if one fails, it affects the whole chain. 'Micro' means every single panel gets its own tiny inverter, so they work independently, which is great but expensive. 'Optimizer' is a middle ground where we make the panels smart enough to talk to a central inverter. Choosing the right one balances cost with how much control and reliability you generally need for the system.">Inverter Topology</label>
        ...
        <div className="glass-panel p-4 rounded-3 mb-4">
            ...
            <div className="row g-3">
                <div className="col-md-4">
                    ...
                    <div className="input-group">
                        <input type="number" className="form-control" value={data.roofPitchDegrees} onChange={e => formStore.setKey('roofPitchDegrees', parseFloat(e.target.value))} title="This is simply the slope or steepness of the roof where the panels will sit, measured in degrees. A flat commercial roof is near 0 degrees; a steep house roof might be 30 or 45 degrees. This matters for two reasons: physics and sunshine. Steeper roofs catch more wind (like a sail), so we need stronger bolts. But a steeper roof might also face the sun better, catching more rays. We need to enter this accurately so the structural engineer knows how much wind force to design for and the yield model knows how much sun the panels will actually see." />
                ...
                <div className="col-md-4">
                    ...
                    <div className="input-group">
                        <input type="number" className="form-control" value={data.windUpliftResistance} onChange={e => formStore.setKey('windUpliftResistance', parseFloat(e.target.value))} title="When wind blows over a solar panel on a roof, it creates suction that tries to rip the panel off, just like an airplane wing generates lift. This 'Uplift Resistance' is the strength rating of the roof and our attachment system—how hard the wind can pull before things break. If you are in a hurricane zone, this number needs to be huge, requiring more screws and heavier steel. We track this to ensure the design meets local building codes and that your investment doesn't literally fly away in the first big storm." />
        ...
        <div className="glass-panel p-4 rounded-3 mb-4">
            ...
            <div className="row g-3">
                ...
                <div className="col-md-6">
                    ...
                    <div className="input-group">
                        <span className="input-group-text bg-transparent text-light border-light">$</span>
                        <input type="number" className="form-control" value={data.balloonPayment} onChange={e => formStore.setKey('balloonPayment', parseFloat(e.target.value))} title="In some loans, you pay small monthly payments for a few years, but you still owe a huge chunk of money at the end—that final big check is the 'Balloon Payment'. This field asks how much that lump sum will be in Year 7. Banks do this to lower your monthly bills so you have more cash flow now, assuming you'll refinance or sell the project later to pay off the balloon. It can be risky; if you can't come up with that big pile of cash in Year 7, the bank could take your solar plant. We need to plan for this cash 'cliff'." />
                    </div>
                </div>
            </div>
        </div>
...
const Step8_Tax = () => {
    ...
    return (
        <div className="glass-panel p-4 rounded-3 mb-4">
            ...
            <div className="mb-3">
                <label className="form-label text-light" title="The government lets businesses write off the cost of equipment as a tax deduction over time—this is 'depreciation'. For solar, you get a special fast-lane deal called 'MACRS-5'. It lets you deduct the entire cost of the solar plant from your taxes in just 5 years, even though the plant lasts 30 years. This creates huge tax savings immediately, which is like getting cash back early. The 'Straight-Line' option spreads it out evenly over more years, which is slower. Choosing MACRS usually boosts your short-term profit significantly, making the investment much more attractive to tax-equity investors.">Depreciation MACRS Schedule</label>
                <div className="btn-group w-100" role="group">
            ...
            <div className="row g-3">
                <div className="col-md-6">
                    ...
                    <div className="input-group">
                        <input type="number" className="form-control" value={data.itcPercentage} onChange={e => formStore.setKey('itcPercentage', parseFloat(e.target.value))} title="The Investment Tax Credit (ITC) is a federal coupon off the price of your solar system. The base coupon is usually 30%, but you can get bonus 'adders'—like 10% more for using US-made steel (Domestic Content) or 10% for building in a low-income area. This input is the *total* percent you expect to claim (e.g., 30%, 40%, or even 50%+). It's incredibly powerful; a 40% ITC effectively means the government pays for nearly half your project through tax refunds. We need to get this right because it's typically the single biggest source of value in the entire deal structure." />
                        <span className="input-group-text bg-transparent text-light border-light">%</span>
                    </div>
        ...
        <div className="glass-panel p-4 rounded-3 mb-4">
            ...
            <div className="row g-3 mb-3">
                <div className="col-md-6">
                    ...
                    <div className="input-group">
                        <input type="number" className="form-control" value={data.dataAcquisitionIntervalSeconds} onChange={e => formStore.setKey('dataAcquisitionIntervalSeconds', parseInt(e.target.value))} title="This is how often we check the solar plant's vitals— 'Are you running? How much power? Any errors?' calling home. Checking every 1 second gives you incredible real-time data but creates a mountain of files to store and analyze (expensive storage). Checking every 15 minutes saves space but means you might not notice a broken inverter until it's been down for a while. We set this interval (in seconds) to balance the need for operational visibility against the cost of data management and bandwidth. For a large plant, fast polling is critical to catch grid blips." />
        ...
        <div className="glass-panel p-4 rounded-3 mb-4 border-warning">
            ...
            <div className="row g-3 mb-4">
                ...
                <div className="col-12">
                    ...
                    <div className="input-group">
                        <span className="input-group-text bg-transparent text-light border-light">$</span>
                        <input type="number" className="form-control" value={data.inverterReplacementReserve} onChange={e => formStore.setKey('inverterReplacementReserve', parseFloat(e.target.value))} title="Inverters (the boxes converting power) act like computers; they wear out faster than the solar panels themselves. Typically, they die around year 10-15. This 'Reserve' is a savings account where we tuck away a little money every year (per kW) so that when the inverters inevitably fail, we have a pile of cash ready to buy new ones without asking investors for more money. It's prudent financial planning. If we save too little (e.g., $0), the project looks profitable now but will go bankrupt in Year 15 when the equipment breaks." />
